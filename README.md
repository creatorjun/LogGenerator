...
각 파일은 배치 경계를 유지하면서 약 1 MiB 단위로 회전합니다. 로그 이벤트 중간 절단을 피하기 때문에 파일 크기는 1 MiB보다 조금 작을 수 있습니다. FILE은 디스크 순차 처리량과 생성 순서를 유지하기 위해 Worker를 단일 writer로 고정하며 목표 EPS 설정은 동일하게 적용됩니다. 총 생성량, 파일 개수, 실행 시간 제한의 기본값은 모두 0(무제한)입니다. 필요한 제한만 0보다 큰 값으로 지정할 수 있으며 먼저 도달한 제한에서 오류 없이 정상 종료합니다.
