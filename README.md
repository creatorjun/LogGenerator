...
각 로그 이벤트는 크기와 관계없이 독립된 파일 하나로 저장됩니다. FILE은 생성 순서를 유지하기 위해 Worker를 단일 writer로 고정하며 목표 EPS 설정은 동일하게 적용됩니다. 총 생성량, 파일 개수, 실행 시간 제한의 기본값은 모두 0(무제한)입니다. 필요한 제한만 0보다 큰 값으로 지정할 수 있으며 먼저 도달한 제한에서 오류 없이 정상 종료합니다.
...
- FILE은 단일 순차 writer가 로그 이벤트마다 독립 파일을 생성하고 기록 전에 총량·개수·시간 제한을 검사합니다.
